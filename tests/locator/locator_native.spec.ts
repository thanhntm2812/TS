...
test("Menu list > Store list - NATIVE ", async ({ page }) => {
  ...
  await passwordTextbox.fill("Asecreat");
  ...
  const statusList = page
    .locator("div.ant-select-item-option-content")
    .filter({ hasText: "Khởi tạo" });
  ...
  const customerTypeOption = page
    .locator("div.ant-select-item-option-content")
    .filter({ hasText: "INDIRECT" });
  ...
  const saleChannelOption = page
    .locator("span.ant-pro-select-item-option-content-light")
    .filter({ hasText: "CHUNG" });
